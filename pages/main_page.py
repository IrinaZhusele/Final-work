#!/usr/bin/python3
# -*- encoding=utf8 -*-

import os, pickle
from telnetlib import EC

from selenium.webdriver.common.by import By
from selenium.webdriver.support.wait import WebDriverWait
from selenium.webdriver.support import expected_conditions as EC

from config.config import TestData
from pages.base import WebPage
from pages.elements import WebElement
from pages.elements import ManyWebElements
from typing import List

from pages.product_page import ProductPage
from pages.shopping_cart_page import ShoppingCartPage


class MainPage(WebPage):
    contact_lang_info = WebElement(xpath='//div[contains(@class,"contactslang")]')
    lang_urls = ManyWebElements(xpath='//div[contains(@class,"lang")]/div/a')
    links_urls = ManyWebElements(xpath='//div[contains(@class,"links")]//a')
    menu = WebElement(css_selector='div[class*="menu"]')
    menu_urls = ManyWebElements(xpath='//div[contains(@class,"menu")]//a')
    search = WebElement(id='search')
    products = ManyWebElements(xpath='(//div[contains(@class,"products")])[1]/div')
    products_titles = ManyWebElements(xpath='(//div[contains(@class,"products")])[1]/div//dt')
    products_images = ManyWebElements(
        xpath='(//div[contains(@class,"products")])[1]/div/a/div[@class="product"]/img')
    products_prices = ManyWebElements(
        xpath='(//div[contains(@class,"products")])[1]/div/a/div[@class="product"]/div')
    products_buy_btns = ManyWebElements(
        xpath='//button[contains(@class, "buy")]')
    pages_links = ManyWebElements(xpath="//div[@class='pagination']//li/a")
    logo_footer_info = WebElement(xpath='//div[contains(@class,"logo")]')
    footer_info = ManyWebElements(xpath='//div[contains(@class,"copyright")]//a')
    categories_links = ManyWebElements(css_selector='#categories>h3')
    subcategories_default_links = ManyWebElements(xpath='//div[@id="categories"]/a[not(h3)]')
    cart = WebElement(xpath='//a[@id="cart"]')
    social_network = ManyWebElements(xpath='//div[contains(@class,"icons")]/a')
    facebook = WebElement(xpath='//div[contains(@class,"icon")]/a[1]')
    instagram = WebElement(xpath='//div[contains(@class,"icon")]/a[2]')
    whatsup = WebElement(xpath='//div[contains(@class,"icon")]/a[3]')

    def __init__(self, web_driver):
        super().__init__(web_driver, TestData.BASE_URL)

    def get_lang_element(self, lang):
        return WebDriverWait(self._web_driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//a[@hreflang='{l}']".format(l=lang))))

    def get_sub_category_element(self, name):
        return WebDriverWait(self._web_driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, "//div[@id='categories']//a[contains(string(),'{n}')]".format(n=name))))

    def is_search_visible(self):
        return self.search.is_visible()

    def is_contact_lang_info_visible(self):
        return self.contact_lang_info.is_visible()

    def languages_count(self):
        return self.lang_urls.count()

    def is_language_with_path_visible(self, url_path):
        urls = self.lang_urls.get_attribute('href')
        matching = [s for s in urls if url_path in s]
        return len(matching) > 0

    def links_count(self):
        return self.links_urls.count()

    def is_link_with_path_visible(self, url_path):
        urls = self.links_urls.get_attribute('href')
        matching = [s for s in urls if url_path in s]
        return len(matching) > 0

    def get_menu_count(self):
        return self.menu_urls.count()

    def is_menu_with_path_visible(self, url_path, position):
        urls = self.menu_urls.get_attribute('href')
        return url_path in urls[position - 1]

    def buy_first_product(self):
        products = self.products_buy_btns.find()
        products[0].click()
        return products[0]

    def get_products_count(self):
        return self.products.count()

    def get_product_cards(self):
        return self.products

    def get_product_buy_btns(self):
        return self.products_buy_btns

    def click_buy_button_at(self, index):
        button = self.products_buy_btns[index]
        WebDriverWait(self._web_driver, 10).until(EC.element_to_be_clickable((By.CLASS_NAME, 'buy')))
        button.click()

    def click_product_at(self, index):
        self.products[index].click()
        return ProductPage(web_driver=self._web_driver)

    def get_product_titles(self):
        return self.products_titles

    def get_product_images(self):
        return self.products_images

    def get_product_prices_elements(self):
        return self.products_prices

    def get_product_price_at(self, index):
        return float(self.products_prices[index].text.replace('€', ''))

    def get_product_title_at(self, index):
        return self.products_titles[index].text

    def click_lang(self, lang):
        element = self.get_lang_element(lang)
        element.click()

    def click_shopping_cart(self):
        self.cart.click()
        return ShoppingCartPage(web_driver=self._web_driver)

    def change_page(self, page):
        element_texts = self.pages_links.get_text()
        index = element_texts.index(str(page))
        element = self.pages_links[index]
        element.click()

    def get_categories_links(self):
        return self.categories_links

    def get_default_subcategories_links(self):
        return self.subcategories_default_links

    def is_logo_footer_info_visible(self):
        return self.logo_footer_info.is_visible()

    def footer_info_count(self):
        return self.footer_info.count()

    def is_footer_info_with_path_visible(self, url_path):
        urls = self.footer_info.get_attribute('href')
        matching = [s for s in urls if url_path in s]
        return len(matching) > 0

    def social_icons_count(self):
        return self.social_network.count()

    def is_social_icon_with_path_visible(self, url_path):
        icons = self.social_network.get_attribute('href')
        matching = [s for s in icons if url_path in s]
        return len(matching) > 0

    def get_facebook_url(self):
        return self.facebook.get_attribute('href')

    def get_instagram_url(self):
        return self.instagram.get_attribute('href')

    def get_whatsup_url(self):
        return self.whatsup.get_attribute('href')

    # def add_product_to_cart(self):
    #     self.products.

    # def first_test():
    #     options = webdriver.ChromeOptions()
    #     options.add_experimental_option("excludeSwitches", ["enable-logging"])
    #     driver = webdriver.Chrome(options=options, executable_path=r'C:/Users/…/…/chromedriver.exe')
    #     driver.get("https://www.saucedemo.com/")
    #
    #     # Поиск элементов и присваивание к переменным.
    #     input_username = driver.find_element_by_xpath("//*[@id=\"user-name\"]")
    #     input_password = driver.find_element_by_xpath("//*[@id=\"password\"]")
    #     login_button = driver.find_element_by_xpath("//*[@id=\"login-button\"]")
    #
    #     # Действия с формами
    #     input_username.send_keys("standard_user")
    #     input_password.send_keys("secret_sauce")
    #     login_button.send_keys(Keys.RETURN)
    #
    #     # Поиск ссылки элемента позиции магазина и клик по ссылке
    #     item_name = driver.find_element_by_xpath("//*[@id=\"item_5_title_link\"]/div")
    #     item_name.click()
    #
    #     # Поиск кнопки добавления товара и клик по этой кнопке
    #     item_add_button = driver.find_element_by_xpath("//*[@id=\"add-to-cart-sauce-labs-fleece-jacket\"]")
    #     item_add_button.click()
    #
    #     # Поиск кнопки коризины и клик по этой кнопке
    #     shopping_cart = driver.find_element_by_xpath("//*[@id=\"shopping_cart_container\"]/a")
    #     shopping_cart.click()
    #
    #     # Еще один поиск ссылки элемента позиции магазина
    #     item_name = driver.find_element_by_xpath("//*[@id=\"item_5_title_link\"]/div")
    #     if item_name.text == "Sauce Labs Fleece Jacket":
    #         print("Товар пристутствует в корзине")
    #     else:
    #         print("Товар отсутствует")
    #
    #     time.sleep(5)
    #
    # if __name__ == '__main__':
    #     first_test()

    # # Main search field
    # search = WebElement(id='header-search')
    #
    # # Search button
    # search_run_button = WebElement(xpath='//*[@id="minwidth"]/div[4]/div/div[1]/div[2]/div/ul/li[4]/a/span[2]/span[1]/span')
    #
    # # Search button
    # search_run_button = WebElement(xpath='//*[@id="authorize"]/div[1]/div[1]/div[1]/div[1]/div[2]/div[1]/a[1]')
    #
    # # Search button
    # search_run_button = WebElement(xpath='//*[@id="authorize"]/div[1]/div[1]/div[1]/div[1]/div[2]/div[2]/div[1]/ul[1]/li[5]/a[1]/span[1]')
    #
    # # Titles of the products in search results
    # products_titles = ManyWebElements(xpath='//a[contains(@href, "/product-") and @title!=""]')
    #
    # # Button to sort products by price
    # sort_products_by_price = WebElement(css_selector='button[data-autotest-id="dprice"]')
    #
    # # Prices of the products in search results
    # products_prices = ManyWebElements(xpath='//div[@data-zone-name="price"]//span/*[1]')
    # # Last pet element:
    # last_pet = WebElement(
    #     xpath="//img[contains(@src,'data:image/jpeg;base64,')]")
    # screenshot = ManyWebElements()
